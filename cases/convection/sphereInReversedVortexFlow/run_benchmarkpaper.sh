...
argo-initialize-parameter-study.py benchmark- -m blockMesh -f initFields.sh -par
...
nohup argo-run-study.py interFoam -d benchmark-interFoam -np 32 > logs.interFoam &
nohup argo-run-study.py interIsoFoam -d benchmark-interIsoFoam -np 32 > logs.interIsoFoam &
